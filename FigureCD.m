...
%log_util_mct(u)=100*log_util_mc(u,tt(u))/log_util_sp(u);
...
%envy plot
t=tiledlayout(1,3);
title1=strcat('Scenario C, n= ',num2str(n), ', d= ',num2str(d),', dependent valuations. Comp. times: MCT+=',num2str(sum(sum(time_mc,2))/SIM+add,'%.2f'),' sec., SPL=',num2str(mean(time_sp),'%.2g'),' sec.');
...
saveas(gcf,'../Figures/FigureCD.pdf')
