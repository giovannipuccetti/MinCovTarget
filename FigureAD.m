...
%envy plot
t=tiledlayout(1,3);
title1=strcat('Scenario A, n= ',num2str(n), ', d= ',num2str(d),', dependent valuations. Comp. times: MCT+=',num2str(sum(sum(time_mc,2))/SIM+add,'%.2f'),' sec., SPL=',num2str(mean(time_sp),'%.2g'),' sec.');
title(t,title1, 'FontSize', 38)
%t.TileSpacing = 'compact';
nexttile
Label1=strcat('MCT+, avg=',num2str(mean(envy(:,1)),'%.2f'));
Label2=strcat('SPL, avg=',num2str(mean(envy(:,2)),'%.4g'));
...
saveas(gcf,'../Figures/FigureAD.pdf')
